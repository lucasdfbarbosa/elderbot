...
client.on('message', function (message: any) {
  ...
  if (command === 'status') {
    if (
    ...
    } else if (!idInvestigadores.includes(args[0])) {
      message.reply(
      ...
      result.addFields(
        ...
        {
          name: 'Sanidade 3.0',
          value: `${negrito(mesa[imatv].cinvs[x].sanid.atual)} ${Emojis.SANIDADE}`,
          inline: true,
        },
        {
          name: 'Resistência',
          value: negrito(mesa[imatv].cinvs[x].resist.atual) + ' ' + Emojis.RESISTENCIA,
          inline: true,
        },
        { name: '\u200B', value: '\u200B' },
        {
